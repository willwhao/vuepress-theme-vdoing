...
#### 1.3 load 事件，error 事件 ，abort 事件 （页面或某个资源加载成功/失败/取消时调用）
...
`pagehide`事件实例也**有一个`persisted`属性，将这个属性设为`true`，就表示页面要保存在缓存中；设为`false`，表示网页不保存在缓存中**，这时如果设置了 unload 事件的监听函数，该函数将在 pagehide 事件后立即运行。
...
#### 2.3 hashchange 事件 （hash 发生变化时触发）
...
#### 3.1 DOMContentLoaded 事件 （DOM 内容加载完成后触发）
